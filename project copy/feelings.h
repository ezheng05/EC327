...
extern vector<pair<string, string>> feeling_entries; // global vec to store feelings & energies
...
void createbutton(sf::Text &button, sf::Font &font, const string &button_name, float xpos, float ypos){
    button.setFont(font);
...
// add feeling entries and remove first if more than 7
void addFeel(string &feeling, string &energy) {
    feeling_entries.push_back({feeling,energy});
}

vector<pair<string, string>> getLastFeel() {
    vector<pair<string, string>> last_entries;
    int count = 0;
    for (auto i = feeling_entries.rbegin(); i != feeling_entries.rend() && count < 7; ++i, ++count) {
        last_entries.push_back(*i);
    }
    return last_entries;
}
...
int feelings(){
    ...
    sf::Texture image;
     if(!image.loadFromFile("feelings/pic_teddy.jpeg")){
